...
Sequence:: Sequence(){
    maxSize = DEFAULT_MAX_ITEMS;//default constructor allows array to hold max
    sizeOfArray = 0;
    seq = new ItemType [maxSize];
}

Sequence:: Sequence(int numOfItems){
    maxSize = numOfItems;
    sizeOfArray = 0;
    seq = new ItemType [numOfItems]; //construct the array with the size
}

Sequence:: ~Sequence (){ //destructor
    delete [] seq; //deletes array
}


Sequence:: Sequence (const Sequence& old){ //copy constructor
    sizeOfArray = old.size(); //set size to size of sequence to be copied
    maxSize = old.maxSize;
    seq = new ItemType [sizeOfArray];
    for (int j = 0; j < old.size(); j++){
        seq[j] = old.seq[j]; //copying contents of array
    }
}
...
int Sequence :: insert(int pos, const ItemType& value){
    ...
    ItemType temp2;
    seq[pos] = value;
    for (int i = pos+1; i < size() + 1; i++){ //shift everything right
        temp2 = seq[i];
        seq[i] = temp;
        temp = temp2;
    }
...
void Sequence:: swap(Sequence& other){
    ...
    int tempMax = maxSize;
    maxSize = other.maxSize; //swap max sizes 
