...
int countTrue(const double a[], int n)
{
    int counter = 0;
    if (n<= 0) return false;
    if (somePredicate(a[0]) == true) counter ++;
    
    return (counter += countTrue (a+1, n-1));
...
int firstTrue(const double a[], int n)
{
    ...
    if (n<= 0) return -1;
    if (somePredicate(a[0]) == true) return 0;
...
bool isIn(const double a1[], int n1, const double a2[], int n2)
{
    ...
    if (n1 == 0) return true;
    if (n1 < 0 || n2 <= 0) return false; //invalid length parameters
    
    if (a1[0] == a2[0]){
        return (isIn(a1+1, n1-1, a2+1, n2-1)); //shorten both arrays if elements match
    }
    else return isIn(a1, n1, a2+1, n2-1); //shorten a2 array if current
}
