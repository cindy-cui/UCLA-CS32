...
int countIsIn(const double a1[], int n1, const double a2[], int n2)
{
   ...
   int counter = 0;
    ...
    if (a1[0] == a2[0]){
       counter += countIsIn(a1+1, n1-1, a2+1, n2-1)+ countIsIn(a1, n1, a2+1, n2-1); //shorten both and then search for a1 in rest of a2
    }
    else counter+= countIsIn(a1, n1, a2+1, n2-1); //only shorten a2 array
...
void order(double a[], int n)
{
    ...
    int x; //firstNotGreater
    int y; //firstLess
    double middleElem = a[n/2]; //divider
    
    divide(a, n, middleElem, x, y); //split in half by divider which is the middle element of current array
    order (a, x + 1); //call order on first half
    order (a + y, n-y); //call order on second half
