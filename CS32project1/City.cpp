...
City::City(int nRows, int nCols)
 : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nFlatulans(0), m_history(m_rows, m_cols)
{
...
History &City:: history() {
    return m_history; //return history object
}
...
void City::preachToFlatulansAroundPlayer()
{
    ...
    bool atleastoneunsuccesful = false; //track if theres a failed preaching
    if (m_player == nullptr)
        return;
    ...
    for (int k = 0; k < m_nFlatulans; )
    {
            ...
            fp->possiblyGetConverted() )
        {
            ...
            m_nFlatulans--;
                 ...
                 coldiff >= -1  &&  coldiff <= 1){ //when the conversion is false
            atleastoneunsuccesful = true;
            k++;
        }
        else
            k++;
    ...
    if (atleastoneunsuccesful == true) { //when there's at least one unconverted flatulan
        m_history.record(m_player->row(), m_player->col()); //record when get not converted but adjacent
    }
}
